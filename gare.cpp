...
void gare::arreterSur(joueur* ptjoueur){
	if (proprietaire =="none"){
		...
		if (reponse == Oui) {
			...
			cout<<"Vous avez acheté la propriété "<< this->nom << ". Votre solde est maintenant de " << ptjoueur.getSolde;
		...
		ptjoueur.setSolde(ptjoueur.getSolde - this->loyer); //le joueur paye le locataire le prix du loyer
